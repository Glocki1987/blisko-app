...
function App() {
  ...
  return <div className="app-shell">
    ...
    {selectedChat && activeChat ? <ChatView chat={activeChat} onBack={() => setSelectedChat(null)} onSent={refresh} /> : <main className="main-content">
      ...
      {tab === 'profile' && <ProfileView profile={profile} online={online} onSave={saveProfile} onDelete={deleteProfile} onOpenUtility={setUtility} />}
    </main>}
...
function ProfileView({ profile, online, onSave, onDelete, onOpenUtility }: { profile: Profile; online: boolean; onSave: (profile: Profile) => Promise<void>; onDelete: () => Promise<void>; onOpenUtility: (type: 'notifications' | 'privacy' | 'help') => void }) {
...
function ProfileDetail({ profile, onClose, onLike, onMessage }: { profile: Profile; onClose: () => void; onLike: () => void; onMessage: () => void }) { const mode = datingModes.find(item => item.value === profile.datingMode) || datingModes[2]; return <div className="profile-modal" onClick={onClose}><article onClick={event => event.stopPropagation()}><button className="modal-close" onClick={onClose}>×</button><img src={profile.image} alt={profile.name} /><div className="modal-copy"><p className="eyebrow">ПРОФИЛЬ</p><h2>{profile.name}, {profile.age}</h2><p><MapPin size={14} /> {profile.city} <span className={profile.online ? 'modal-online' : 'modal-recent'}>● {profile.online ? 'в сети' : 'был(а) недавно'}</span></p><span className="mode-badge">{mode.icon} {mode.label}</span><p className="modal-bio">{profile.bio || 'Пока без описания.'}</p><div className="modal-interests">{profile.tags.length ? profile.tags.map(tag => <span key={tag}>#{tag}</span>) : <small>Интересы пока не добавлены</small>}</div><button className="primary wide" onClick={onLike}><Heart size={18} fill="currentColor" /> Нравится</button><button className="secondary wide" onClick={onMessage}><MessageCircle size={18} /> Написать</button></div></article></div>; }
