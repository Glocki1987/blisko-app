...
function Onboarding({ profile, onDone }: { profile: Profile; onDone: (profile: Profile) => void }) {
  ...
  return <div className="onboarding"><div className="onboard-art"><div className="blob blob-a" /><div className="blob blob-b" /><Heart className="onboard-heart" fill="white" size={48} /></div><div className="onboard-copy"><div className="brand big">bli<span>s</span>ko</div><div className="onboarding-progress"><span>Шаг {step} из 4</span><div><i style={{ width: `${step * 25}%` }} /></div></div><h1>{step === 1 ? 'Расскажи о себе' : step === 2 ? 'Кого хочешь встретить?' : step === 3 ? 'Где ты живёшь?' : 'Добавь настроение'}</h1><p>{step === 1 ? 'Так люди узнают, как к тебе обращаться.' : step === 2 ? 'Настроим ленту под твои интересы.' : step === 3 ? 'Покажем подходящих людей рядом.' : 'Фото и несколько слов сделают анкету живой.'}</p><form className="onboard-form" onSubmit={event => { event.preventDefault(); if (step === 4) onDone(form); else next(); }}>{step === 1 && <><input autoFocus required value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} placeholder="Как тебя зовут?" /><input required type="number" min="18" max="100" value={form.age} onChange={event => setForm({ ...form, age: Number(event.target.value) })} placeholder="Возраст" /></>}{step === 2 && <div className="choice-grid"><button type="button" className={form.gender === 'female' ? 'choice selected' : 'choice'} onClick={() => setForm({ ...form, gender: 'female' })}>Я девушка</button><button type="button" className={form.gender === 'male' ? 'choice selected' : 'choice'} onClick={() => setForm({ ...form, gender: 'male' })}>Я парень</button><button type="button" className={form.interestedIn === 'female' ? 'choice selected' : 'choice'} onClick={() => setForm({ ...form, interestedIn: 'female' })}>Ищу девушек</button><button type="button" className={form.interestedIn === 'male' ? 'choice selected' : 'choice'} onClick={() => setForm({ ...form, interestedIn: 'male' })}>Ищу парней</button><button type="button" className={form.interestedIn === 'all' ? 'choice selected' : 'choice'} onClick={() => setForm({ ...form, interestedIn: 'all' })}>Ищу всех</button></div>}{step === 3 && <select autoFocus required value={form.city} onChange={event => setForm({ ...form, city: event.target.value })}><option value="">Выбери город в Польше</option>{polishCities.map(city => <option key={city}>{city}</option>)}</select>}{step === 4 && <><label className="photo-picker">{form.image ? <img src={form.image} alt="Фото профиля" /> : <span>＋</span>}<input type="file" accept="image/*" onChange={event => setPhoto(event.target.files?.[0])} /><small>{form.image ? 'Изменить фото' : 'Добавить фото'}</small></label><textarea maxLength={500} value={form.bio} onChange={event => setForm({ ...form, bio: event.target.value })} placeholder="Расскажи о себе (до 500 символов)" /><small className="form-tip">Фото можно добавить сейчас или позже в профиле.</small></>}<div className="onboarding-actions">{step > 1 && <button type="button" className="secondary" onClick={back}>Назад</button>}<button className="primary wide" type="submit" disabled={!canContinue}>{step === 4 ? 'Создать анкету' : 'Продолжить'} <ChevronRight size={19} /></button></div></form></div></div>;
}
